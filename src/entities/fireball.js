...
export class Fireball {
  ...
  spawn(edge,color='normal',bounds=FULL_BOUNDS){
    ...
    this.last=edge;
    const b=bounds,m=170,w=b.right-b.left,h=b.bottom-b.top;
    const positions=[
  ...
  draw(x,img){
    ...
    const hue=this.color==='purple'?245:this.color==='pink'?305:this.color==='green'?90:0;
    const glowColor=this.color==='purple'?'rgba(172,65,255,':this.color==='pink'?'rgba(255,66,175,':this.color==='green'?'rgba(70,255,110,':'rgba(255,115,0,';
    ...
    x.filter=`blur(${(1-ease)*2}px) hue-rotate(${hue}deg) saturate(${hue?1.45:1})`;
